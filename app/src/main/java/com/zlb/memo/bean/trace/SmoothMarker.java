package com.zlb.memo.bean.trace;
        ...
        moveThread = new Thread(new Runnable() {
            ...
            private void startRun() {
                try {
                    ...
                    //中心焦点移到
                    Point screempoint = mAMap.getProjection().toScreenLocation(curPos);
                    if (screempoint.x < 0 || screempoint.x > C.screenWidth || screempoint.y < 0 || screempoint.y > C.screenHeight) {
                        mAMap.animateCamera(CameraUpdateFactory.changeLatLng(curPos));
                    }
                } catch (Throwable e) {
                    e.printStackTrace();
                }
